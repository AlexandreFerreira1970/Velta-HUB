...
const BackIcon = () => (
  <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <path d="M9 11L5 7l4-4" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
)
...
function AnalysisCard({ doc, onView }: { doc: HubDataDoc; onView: () => void }) {
  ...
  return (
    <button
      ...
      style={{ background: 'var(--surface-0)', borderColor: 'var(--steel-soft)', cursor: 'pointer' }}
      onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--steel)' }}
      onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--steel-soft)' }}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-1.5">
          <Text variant="caption" color="tertiary">{formatDate(doc.createdAt)}</Text>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-medium" style={{ color: scoreColor(hubScore) }}>
              {scoreLabel(hubScore)}
            </span>
          </div>
        </div>
      ...
      <div className="mt-4 grid grid-cols-5 gap-2">
        {(['rh', 'fin', 'log', 'mkt', 'esg'] as const).map((dim) => (
          <div key={dim} className="flex flex-col gap-0.5">
            <Text variant="overline" color="tertiary">{dim.toUpperCase()}</Text>
            <span
              className="text-sm font-semibold tabular-nums"
              style={{ color: scoreColor(doc.output.adjustedDimensions[dim]), fontFamily: 'var(--font-geist-mono, monospace)' }}
            >
...
export default function HubHistoryPage() {
  ...
  const handleView = (doc: HubDataDoc) => {
    sessionStorage.setItem('hub_result', JSON.stringify(doc.output))
    ...
    <div className="flex flex-col min-h-screen" style={{ background: 'var(--canvas)' }}>
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span style={{ color: 'var(--steel)', fontSize: 16, userSelect: 'none' }}>|</span>
          <Text variant="body-sm" color="secondary">Histórico</Text>
          <span style={{ color: 'var(--steel)', fontSize: 16, userSelect: 'none' }}>|</span>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-8">
        <div className="max-w-2xl mx-auto">
          ...
          {error && (
            <p className="text-sm text-center py-12" style={{ color: 'var(--semantic-error)' }}>
              {error}
            </p>
          )}
          ...
          {!history && !error && (
            <div className="py-12 text-center">
              <Text variant="body-sm" color="tertiary">Carregando histórico...</Text>
            </div>
          )}

          {history && history.length === 0 && (
            <div className="py-12 text-center">
              <Text variant="body-sm" color="tertiary">Nenhuma análise realizada ainda.</Text>
            </div>
          )}
          ...
          {history && history.length > 0 && (
            <div className="space-y-3">
              {history.map((doc) => (
                <AnalysisCard key={doc.id} doc={doc} onView={() => handleView(doc)} />
              ))}
            </div>
          )}
        </div>
