...
import type { DecisionOutput, ScenarioClassification, DimensionScores, DimensionWeights } from '@/lib/types'
...
const SCENARIO_CONFIG: Record<ScenarioClassification, { label: string; bg: string; color: string }> = {
  bloqueio_automatico:  { label: 'Bloqueio Automático',  bg: 'var(--semantic-error)',   color: '#fff' },
  prioridade_critica:   { label: 'Prioridade Crítica',   bg: 'var(--semantic-warning)', color: '#000' },
  aprovacao_obrigatoria:{ label: 'Aprovação Obrigatória',bg: 'var(--navy)',              color: '#fff' },
  execucao_assistida:   { label: 'Execução Assistida',   bg: 'var(--azure)',             color: '#fff' },
  execucao_automatica:  { label: 'Execução Automática',  bg: 'var(--semantic-success)',  color: '#fff' },
}

const DIMENSION_LABELS: Record<keyof DimensionScores, string> = {
  rh: 'RH', fin: 'FIN', log: 'LOG', mkt: 'MKT', esg: 'ESG',
}
...
function DimensionCard({
  ...
  return (
    ...
    >
      <Text variant="overline" color="tertiary">{label}</Text>
      <span
        className="text-2xl font-semibold tabular-nums"
        style={{ color: scoreColor(adjusted), fontFamily: 'var(--font-geist-mono, monospace)' }}
      >
        {adjusted.toFixed(1)}
      </span>
      {hasDelta && (
        <Text variant="caption" style={{ color: 'var(--semantic-warning)' }}>
          {raw.toFixed(1)} → {adjusted.toFixed(1)} ({delta > 0 ? '+' : ''}{delta.toFixed(1)})
        </Text>
      )}
    </div>
  )
    ...
    <div className="flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <span
          ...
          style={{ color: scoreColor(100 - value), fontFamily: 'var(--font-geist-mono, monospace)' }}
        >
          {value.toFixed(1)}
        </span>
        <span
      ...
      <div className="flex flex-wrap gap-3">
        {dims.map((d) => (
          <div key={d} className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-sm" style={{ background: DIMENSION_COLORS[d] }} />
...
export default function HubResultPage() {
  ...
  useEffect(() => {
    ...
    try {
      setOutput(JSON.parse(raw) as DecisionOutput)
    } catch {
      router.push('/hub')
    }
  }, [router])
  ...
  if (!output) {
    return (
      <div className="flex items-center justify-center h-screen" style={{ background: 'var(--canvas)' }}>
        <Text variant="body-sm" color="tertiary">Carregando resultado...</Text>
      </div>
    )
  }

  const { hubScore, rawDimensions, adjustedDimensions, dynamicWeights, strategicIndices, scenario, actions } = output
      ...
      <main className="flex-1 overflow-y-auto px-4 py-8">
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div
            ...
            style={{ background: 'var(--surface-0)', borderColor: 'var(--steel-soft)' }}
          >
            <Text variant="overline" color="tertiary">HUB SCORE</Text>
            <div
              className="text-7xl font-bold tabular-nums mt-2 leading-none"
              style={{ color: scoreColor(hubScore), fontFamily: 'var(--font-geist-mono, monospace)' }}
            >
              {hubScore.toFixed(1)}
            </div>
            <p className="mt-2 text-sm font-medium" style={{ color: scoreColor(hubScore) }}>
              {scoreLabel(hubScore)}
            </p>
          ...
          {/* Dimension grid */}
          <div>
            <Text variant="subheading" className="mb-3">Scores por Dimensão</Text>
          ...
          >
            ...
            <IndexRow label="IRL — Risco Logístico"      value={strategicIndices.irl} level={strategicIndices.irlLevel} />
            <IndexRow label="IIH — Impacto Humano"       value={strategicIndices.iih} level={strategicIndices.iihLevel} />
            <IndexRow label="IU — Urgência"              value={strategicIndices.iu}  level={strategicIndices.iuLevel} />
          </div>
          ...
          <div
            ...
            style={{ background: 'var(--surface-0)', borderColor: 'var(--steel-soft)' }}
          >
            <Text variant="subheading" className="mb-4">Ações Recomendadas</Text>
            <ul className="space-y-2">
              {actions.map((action) => (
                <li key={action} className="flex items-start gap-2">
                  <span style={{ color: 'var(--azure)', marginTop: 2, fontSize: 16, lineHeight: 1 }}>→</span>
          ...
          <div
            ...
            style={{ background: 'var(--surface-0)', borderColor: 'var(--steel-soft)' }}
          >
            <Text variant="subheading" className="mb-4">Pesos Dinâmicos Aplicados</Text>
          ...
          <div className="flex gap-3 pb-8">
            <button
              ...
              style={{ borderColor: 'var(--steel)', color: 'var(--ink-secondary)', background: 'transparent', cursor: 'pointer' }}
              onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--surface-2)' }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent' }}
            >
              Nova Análise
            ...
            <button
              ...
              style={{ background: 'var(--navy)', color: 'var(--ink-inverse)', cursor: 'pointer' }}
              onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--azure)' }}
